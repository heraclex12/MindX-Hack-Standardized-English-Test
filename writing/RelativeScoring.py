...
class PromptRelevantScore:
    ...
    def predict(self, doc_embeddings, prompt):
        feed_dict = {
            self.batch_doc_encodes: doc_embeddings,
            self.prompt_encodes: prompt,
        }
    ...
    def build_graph(self):
        ...
        self.prompt_encodes = tf.placeholder(shape=[1, None, 768], dtype=tf.float32)
        batch_doc_sent_nums = tf.expand_dims(tf.shape(self.batch_doc_encodes)[1], 0)

        self.prompt_encodes = self.prompt_encodes[0]
        p_shape = tf.shape(self.prompt_encodes)

        self.prompt_encodes = tf.convert_to_tensor(self.prompt_encodes)

        batch_index_o = tf.constant(0)
        batch_prompt_doc_encodes_o = tf.zeros([1, self.bert_emb_dim], dtype=tf.float32)

        def cond(batch_index, batch_prompt_doc_encodes):
            return tf.less(batch_index, 1)

        def body(batch_index, batch_prompt_doc_encodes):
            temp_encodes = tf.concat([self.prompt_encodes, self.batch_doc_encodes[batch_index]], 0)
            batch_prompt_doc_encodes = tf.concat([batch_prompt_doc_encodes, temp_encodes], 0)

            return tf.add(batch_index, 1), batch_prompt_doc_encodes

        _, batch_prompt_doc_encodes = tf.while_loop(cond,
                                                    body,
                                                    [batch_index_o,
        ...
        batch_prompt_doc_encodes = tf.reshape(batch_prompt_doc_encodes[1:], [1, -1, self.bert_emb_dim])
...
init_checkpoint = 'writing/fupugecscore/data/output/prompt_score/model.ckpt-534'
prompt_rel_score = PromptRelevantScore(init_checkpoint)
prompts = np.load('writing/fupugecscore/data/train_data/prompt.npz', allow_pickle=True)['features'][()]
...
if __name__ == '__main__':
    ...
    print(prompt_rel_score.predict(np.expand_dims(embeddings, 0), prompts[1]))